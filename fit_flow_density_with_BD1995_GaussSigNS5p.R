fit_flow_density_with_BD1995_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par1_init = max(data_max_density, par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((tanh((p[1]/V2) - p[2]) + tanh(p[2]))/(1.0 + tanh(p[2])))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((tanh((par1/V2) - par2) + tanh(par2))/(1.0 + tanh(par2)))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
