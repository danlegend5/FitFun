fit_flow_density_with_WG2011B_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Wang model B (WG2011B). The
...
tryCatch(
  ...
  { par2_init = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + V2 + I(V2/(1.0 + exp(p[1]*(V2 - p[2])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    model_obj = gamlss(V3 ~ 0 + V2 + I(V2/(1.0 + exp(par1*(V2 - par2)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
tryCatch(
  { q_0 = 0.0
    ...
    tmpval2 = 1.0 + tmpval1
    dvdk_0 = -model_obj$mu.coefficients[2]*par1*tmpval1/(tmpval2*tmpval2)
    ...
    tmpval3 = model_obj$mu.coefficients[1] + (model_obj$mu.coefficients[2]/tmpval2)
    if (tmpval3 > 0.0) {
      v_ff = tmpval3
    } },
