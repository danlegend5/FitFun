fit_flow_density_with_DC2012B_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 + ((par1*((1.0/V2) - (1.0/par2)))^(-par3)))^(-1.0/par3))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      k_vmax = 0.0
      q_cap = v_ff*k_crit*((1.0 + ((par1*((1.0/k_crit) - (1.0/k_jam)))^(-par3)))^(-1.0/par3))
      v_max = v_ff
