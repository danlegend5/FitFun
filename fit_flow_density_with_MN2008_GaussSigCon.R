fit_flow_density_with_MN2008_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    par1_min = 0.0
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - ((V2/p[3])^p[2]))/(1.0 + (p[1]*((V2/p[3])^p[2]))))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - ((V2/par3)^par2))/(1.0 + (par1*((V2/par3)^par2))))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
