fit_speed_density_with_GD2008kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Ghandehari model with fixed
#              jam density (GD2008kjf). The model component for the noise in the speed-density relationship is defined as independent observations
...
tryCatch(
  ...
  { par1_min = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(log((p[1] + k_jam_use)/(p[1] + V2))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(log((par1 + k_jam)/(par1 + V2))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { reconstructed_model_fit = data.table(V2 = seq(from = 0.0, to = upper_density, length.out = ngrid))
    ...
    if (par1 > 0.0) {
      predicted_values_for_mu[1] = model_obj$mu.coefficients[1]*log((par1 + k_jam)/par1)
    } else {
