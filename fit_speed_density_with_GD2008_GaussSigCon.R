fit_speed_density_with_GD2008_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Ghandehari model (GD2008).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + log(V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(log((p[1] + p[2])/(p[1] + V2))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(log((abs(p[1]) + par2_min_use + abs(p[2]))/(abs(p[1]) + V2))), sigma.formula = ~ 1, family = NO()))
      attach(par2_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + I(log((par1 + par2)/(par1 + V2))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { reconstructed_model_fit = data.table(V2 = seq(from = 0.0, to = upper_density, length.out = ngrid))
    ...
    if (par1 > 0.0) {
      predicted_values_for_mu[1] = model_obj$mu.coefficients[1]*log((par1 + par2)/par1)
    } else {
