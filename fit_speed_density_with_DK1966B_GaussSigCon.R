fit_speed_density_with_DK1966B_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Drake multi-regime model B
#              (DK1966B). The model component for the noise in the speed-density relationship is defined as independent observations that follow a
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + log(V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, log(p[1]/V2), log(p[1]/curr_k_b)), sigma.formula = ~ 1, family = NO()))
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, log(par1/V2), log(par1/par2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
