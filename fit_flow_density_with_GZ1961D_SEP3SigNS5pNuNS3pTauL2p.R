fit_flow_density_with_GZ1961D_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { inner_ccrit_use = data.frame(inner_ccrit_use = inner_ccrit)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(sqrt((V2/p[1])*(1.0 - (V2/p[1])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    model_obj = gamlss(V3 ~ 0 + I(sqrt((V2/par1)*(1.0 - (V2/par1)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
cat('  q_cap:', sprintf('%.8g', 0.5*model_obj$mu.coefficients[1]), '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        sprintf('%.8g', 0.5*model_obj$mu.coefficients[1]), '           # q_cap\n',
