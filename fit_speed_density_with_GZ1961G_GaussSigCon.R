fit_speed_density_with_GZ1961G_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model G (GZ1961G). The
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + log(V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - ((V2/p[2])^(p[1] - 1.0))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - ((V2/(par2_min_use + abs(p[2])))^((par1_min_use + abs(p[1])) - 1.0))), sigma.formula = ~ 1, family = NO()))
      attach(par1_min_use)
      attach(par2_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - ((V2/par2)^(par1 - 1.0))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
