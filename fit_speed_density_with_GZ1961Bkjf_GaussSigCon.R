fit_speed_density_with_GZ1961Bkjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model B with fixed jam
#              density (GZ1961Bkjf). The model component for the noise in the speed-density relationship is defined as independent observations that
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(1.0 - sqrt(V2/k_jam)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
