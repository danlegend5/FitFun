...
cat(sprintf('%.8g', data_min_density), '          # Minimum density in the data\n', file = output_file, append = TRUE)
cat(sprintf('%.8g', data_max_density), '          # Maximum density in the data\n', file = output_file, append = TRUE)
if (fd_type == 'Flow.Density') {
  cat(sprintf('%.8g', data_min_y), '          # Minimum flow in the data\n', file = output_file, append = TRUE)
  cat(sprintf('%.8g', data_max_y), '          # Maximum flow in the data\n', file = output_file, append = TRUE)
} else if (fd_type == 'Speed.Density') {
  cat(sprintf('%.8g', data_min_y), '          # Minimum speed in the data\n', file = output_file, append = TRUE)
  cat(sprintf('%.8g', data_max_y), '          # Maximum speed in the data\n', file = output_file, append = TRUE)
}
cat('######################################################################################################################\n',
    '# MODEL PARAMETER COUNTS\n',
    '######################################################################################################################\n',
    sprintf('%.6f', npar_mu), '           # No. of free parameters for "mu"\n',
    sprintf('%.6f', npar_sigma), '           # No. of free parameters for "sigma"\n',
    sprintf('%.6f', npar_nu), '           # No. of free parameters for "nu"\n',
    sprintf('%.6f', npar_tau), '           # No. of free parameters for "tau"\n',
    sprintf('%.6f', npar_all), '           # Total no. of free parameters (Npar)\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# FIT QUALITY\n',
    '######################################################################################################################\n',
    sprintf('%.4f', gdev), '           # Global deviance [ -2*ln(Lmax) ]\n',
    sprintf('%.4f', aic), '           # AIC [ -2*ln(Lmax) + 2*Npar ]\n',
    sprintf('%.4f', bic), '           # BIC [ -2*ln(Lmax) + Npar*ln(Ndat) ]\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# FITTED PHYSICAL PARAMETERS\n',
    '######################################################################################################################\n',
    sprintf('%.8g', q_0), '           # Flow at zero density\n',
    sprintf('%.8g', v_ff), '           # Free-flow speed (i.e. the speed as density tends to zero)\n',
    sprintf('%.8g', dvdk_0), '           # Gradient of the speed (with respect to density) at zero density\n',
    sprintf('%.8g', k_crit), '           # Critical density (i.e. the density at maximum flow)\n',
    sprintf('%.8g', k_vmax), '           # Density at maximum speed\n',
    sprintf('%.8g', q_cap), '           # Capacity (i.e. the maximum flow)\n',
    sprintf('%.8g', v_max), '           # Maximum speed\n',
    sprintf('%.8g', k_jam), '           # Jam density (i.e. the density at zero speed)\n',
    sprintf('%.8g', v_bw), '           # Back-propagating wave speed at jam density\n',
    sprintf('%.8g', dvdk_kjam), '           # Gradient of the speed (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    ...
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$q_0), '           # Flow at zero density\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$v_ff), '           # Free-flow speed (i.e. the speed as density tends to zero)\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$dvdk_0), '           # Gradient of the speed (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$k_crit), '           # *Critical density (i.e. the density at maximum flow)\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$k_vmax), '           # *Density at maximum speed\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$q_cap), '           # *Capacity (i.e. the maximum flow)\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$v_max), '           # *Maximum speed\n',
    curve_properties_for_mu_over_data_range$n_peaks, '           # *No. of peaks\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$k_jam), '           # *Jam density (i.e. the density at zero speed)\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$v_bw), '           # *Back-propagating wave speed at jam density\n',
    sprintf('%.8g', curve_properties_for_mu_over_data_range$dvdk_kjam), '           # *Gradient of the speed (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    ...
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$sigma_0), '           # Sigma at zero density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$dsigmadk_0), '           # Gradient of the sigma (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$k_sigmamax), '           # *Density at maximum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$sigma_max), '           # *Maximum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$k_sigmamin), '           # *Density at minimum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$sigma_min), '           # *Minimum sigma\n',
    ...
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$sigma_kjam), '           # Sigma at jam density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_data_range$dsigmadk_kjam), '           # Gradient of the sigma (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "NU" MODEL OVER THE DENSITY RANGE FROM ZERO TO THE MAXIMUM DENSITY IN THE DATA\n',
    '# * = ESTIMATED FOR THE RESTRICTED DENSITY RANGE FROM ZERO TO THE ESTIMATED JAM DENSITY (IF DEFINED)\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$nu_0), '           # Nu at zero density\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$dnudk_0), '           # Gradient of the nu (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$k_numax), '           # *Density at maximum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$nu_max), '           # *Maximum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$k_numin), '           # *Density at minimum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$nu_min), '           # *Minimum nu\n',
    ...
    sprintf('%.8g', curve_properties_for_nu_over_data_range$nu_kjam), '           # Nu at jam density\n',
    sprintf('%.8g', curve_properties_for_nu_over_data_range$dnudk_kjam), '           # Gradient of the nu (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "TAU" MODEL OVER THE DENSITY RANGE FROM ZERO TO THE MAXIMUM DENSITY IN THE DATA\n',
    '# * = ESTIMATED FOR THE RESTRICTED DENSITY RANGE FROM ZERO TO THE ESTIMATED JAM DENSITY (IF DEFINED)\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$tau_0), '           # Tau at zero density\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$dtaudk_0), '           # Gradient of the tau (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$k_taumax), '           # *Density at maximum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$tau_max), '           # *Maximum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$k_taumin), '           # *Density at minimum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$tau_min), '           # *Minimum tau\n',
    ...
    sprintf('%.8g', curve_properties_for_tau_over_data_range$tau_kjam), '           # Tau at jam density\n',
    sprintf('%.8g', curve_properties_for_tau_over_data_range$dtaudk_kjam), '           # Gradient of the tau (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "MU" MODEL OVER THE DENSITY RANGE FROM ZERO TO "upper_density"\n',
    '# * = ESTIMATED FOR THE FIRST RUN OF POSITIVE NUMBERS IN THE "MU" CURVE\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$q_0), '           # Flow at zero density\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$v_ff), '           # Free-flow speed (i.e. the speed as density tends to zero)\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$dvdk_0), '           # Gradient of the speed (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$k_crit), '           # *Critical density (i.e. the density at maximum flow)\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$k_vmax), '           # *Density at maximum speed\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$q_cap), '           # *Capacity (i.e. the maximum flow)\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$v_max), '           # *Maximum speed\n',
    curve_properties_for_mu_over_full_range$n_peaks, '           # *No. of peaks\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$k_jam), '           # *Jam density (i.e. the density at zero speed)\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$v_bw), '           # *Back-propagating wave speed at jam density\n',
    sprintf('%.8g', curve_properties_for_mu_over_full_range$dvdk_kjam), '           # *Gradient of the speed (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "SIGMA" MODEL OVER THE DENSITY RANGE FROM ZERO TO "upper_density"\n',
    '# * = ESTIMATED FOR THE RESTRICTED DENSITY RANGE FROM ZERO TO THE ESTIMATED JAM DENSITY (IF DEFINED)\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$sigma_0), '           # Sigma at zero density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$dsigmadk_0), '           # Gradient of the sigma (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$k_sigmamax), '           # *Density at maximum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$sigma_max), '           # *Maximum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$k_sigmamin), '           # *Density at minimum sigma\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$sigma_min), '           # *Minimum sigma\n',
    ...
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$sigma_kjam), '           # Sigma at jam density\n',
    sprintf('%.8g', curve_properties_for_sigma_over_full_range$dsigmadk_kjam), '           # Gradient of the sigma (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "NU" MODEL OVER THE DENSITY RANGE FROM ZERO TO "upper_density"\n',
    '# * = ESTIMATED FOR THE RESTRICTED DENSITY RANGE FROM ZERO TO THE ESTIMATED JAM DENSITY (IF DEFINED)\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$nu_0), '           # Nu at zero density\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$dnudk_0), '           # Gradient of the nu (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$k_numax), '           # *Density at maximum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$nu_max), '           # *Maximum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$k_numin), '           # *Density at minimum nu\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$nu_min), '           # *Minimum nu\n',
    ...
    sprintf('%.8g', curve_properties_for_nu_over_full_range$nu_kjam), '           # Nu at jam density\n',
    sprintf('%.8g', curve_properties_for_nu_over_full_range$dnudk_kjam), '           # Gradient of the nu (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
cat('######################################################################################################################\n',
    '# ESTIMATED PROPERTIES OF THE FITTED "TAU" MODEL OVER THE DENSITY RANGE FROM ZERO TO "upper_density"\n',
    '# * = ESTIMATED FOR THE RESTRICTED DENSITY RANGE FROM ZERO TO THE ESTIMATED JAM DENSITY (IF DEFINED)\n',
    '######################################################################################################################\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$tau_0), '           # Tau at zero density\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$dtaudk_0), '           # Gradient of the tau (with respect to density) at zero density\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$k_taumax), '           # *Density at maximum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$tau_max), '           # *Maximum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$k_taumin), '           # *Density at minimum tau\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$tau_min), '           # *Minimum tau\n',
    ...
    sprintf('%.8g', curve_properties_for_tau_over_full_range$tau_kjam), '           # Tau at jam density\n',
    sprintf('%.8g', curve_properties_for_tau_over_full_range$dtaudk_kjam), '           # Gradient of the tau (with respect to density) at jam density\n',
    file = output_file, sep = '', append = TRUE)
}
