fit_flow_density_with_GZ1961C_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model C (GZ1961C). The
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + V2 + I(V2^3), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
  ...
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      if (model_obj$mu.coefficients[2] < 0.0) {
        k_jam = sqrt(-v_ff/model_obj$mu.coefficients[2])
        k_crit = (sqrt(3.0)/3.0)*k_jam
        k_vmax = 0.0
        q_cap = (2.0/3.0)*v_ff*k_crit
        v_max = v_ff
        v_bw = 2.0*v_ff
...
cat('  v_ff/(k_jam^2):', -model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        -model_obj$mu.coefficients[2], '           # v_ff/(k_jam^2)\n',
