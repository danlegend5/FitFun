fit_speed_density_with_GZ1961F_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model F (GZ1961F). The
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + I(V2^2), sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
