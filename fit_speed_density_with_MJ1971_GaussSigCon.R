fit_speed_density_with_MJ1971_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Munjal multi-regime model
#              (MJ1971). The model component for the noise in the speed-density relationship is defined as independent observations that follow a
...
tryCatch(
  ...
  { par1_init = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, ((1.0 + p[1])*(curr_k_crit/V2)) - p[1], 1.0), sigma.formula = ~ 1, family = NO()))
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, ((1.0 + par1)*(par2/V2)) - par1, 1.0), sigma.formula = ~ 1, family = NO(), data = traffic_data)
