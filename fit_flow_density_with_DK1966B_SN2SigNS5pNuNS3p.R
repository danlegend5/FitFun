fit_flow_density_with_DK1966B_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              Skew Normal Type II distribution. The density dependence of the log of the scale parameter and the log of the skewness parameter is
#              modelled using natural cubic splines with five and three effective free parameters, respectively (SN2SigNS5pNuNS3p).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, V2*log(p[1]/V2), V2*log(p[1]/curr_k_b)), sigma.formula = ~ ns(V2, df = 4),
    ...
    for (j in 1:par2_nrefine) {
      ...
      for (i in 1:curr_par2_nsteps) {
        ...
        attach(outer_ccrit_use)
        attach(outer_ncyc_use)
        optim_obj = try(find.hyper(model = model_formula, parameters = c(curr_par1_vec[i]), k = 0.0, steps = c(par1_step), lower = c(par1_min), maxit = 500))
        if (class(optim_obj) == 'try-error') { optim_obj = list(convergence = 1) }
        if (optim_obj$convergence != 0) {
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, V2*log(par1/V2), V2*log(par1/par2)), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
