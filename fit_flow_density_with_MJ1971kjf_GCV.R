...
par1_nsteps = 100     # Number of steps to be used for the initial profiling of the free parameter k_crit (must be greater than or equal to 2)
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      ...
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, curr_k_crit*((k_jam - V2)/(k_jam - curr_k_crit)), V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, par1*((k_jam - V2)/(k_jam - par1)), V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    if (model_obj$converged != TRUE) {
