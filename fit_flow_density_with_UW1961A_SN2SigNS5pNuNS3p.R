fit_flow_density_with_UW1961A_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(mu.link = 'log'),
