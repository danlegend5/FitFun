fit_flow_density_with_GZ1961H_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model H (GZ1961H). The
#              model component for the noise in the flow-density relationship is defined as independent observations that follow a Skew Exponential
#              Power Type III distribution. The density dependence of the log of the scale parameter and the log of the skewness parameter is
#              modelled using natural cubic splines with five and three effective free parameters, respectively. The density dependence of the log
#              of the kurtosis parameter is modelled using a straight line function (i.e. an intercept and a gradient as the two free parameters;
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/p[2]))^(1.0/(1.0 - p[1])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    optim_obj = try(find.hyper(model = model_formula, parameters = c(par1_init, par2_init), k = 0.0, steps = c(par1_step, par2_step), lower = c(-Inf, par2_min),
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/(par2_min_use + abs(p[2]))))^(1.0/(1.0 - (par1_max_use - abs(p[1])))))), sigma.formula = ~ ns(V2, df = 4),
                                   nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(), c.crit = outer_ccrit_use, n.cyc = outer_ncyc_use,
                                   i.control = glim.control(cc = inner_ccrit_use, cyc = inner_ncyc_use)))
      attach(par1_max_use)
      attach(par2_min_use)
      optim_obj = try(find.hyper(model = model_formula, parameters = c(par1_max - par1_init, par2_init - par2_min), k = 0.0, steps = c(par1_step, par2_step),
      ...
      par1 = par1_max - abs(optim_obj$par[1])
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/par2))^(1.0/(1.0 - par1)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
...
tryCatch(
  { reconstructed_model_fit = data.table(V2 = seq(from = 0.0, to = min(upper_density, par2), length.out = ngrid))
...
tryCatch(
  { q_0 = 0.0
    v_ff = NA
    dvdk_0 = model_obj$mu.coefficients[1]/(par2*(par1 - 1.0))
    k_crit = NA
    k_vmax = NA
    q_cap = NA
    v_max = NA
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      k_jam = par2
      k_crit = ((1.0 - par1)/(2.0 - par1))*k_jam
      k_vmax = 0.0
      q_cap = v_ff*k_crit*((2.0 - par1)^(1.0/(par1 - 1.0)))
      v_max = v_ff
    } },
...
cat('\n')
cat('Fit quality:\n')
cat('  Global deviance (-2 ln L):   ', format(gdev, nsmall = 4), '\n')
cat('  AIC (-2 ln L + 2 Npar):      ', format(aic, nsmall = 4), '\n')
cat('  BIC (-2 ln L + Npar ln Ndat):', format(bic, nsmall = 4), '\n')
cat('\n')
