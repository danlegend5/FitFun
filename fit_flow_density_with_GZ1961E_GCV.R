fit_flow_density_with_GZ1961E_GCV = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { model_formula = quote(gamlss(V3 ~ 0 + I((V2/p[1])*sqrt(log(p[1]/V2))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    optim_obj = find.hyper(model = model_formula, parameters = c(1.1*data_max_density), k = 0.0, steps = c(par1_step), lower = c(data_max_density), maxit = 500)
    ...
    model_obj = gamlss(V3 ~ 0 + I((V2/par1)*sqrt(log(par1/V2))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
