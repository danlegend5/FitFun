fit_speed_density_with_MJ1971kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Munjal multi-regime model with
#              fixed jam density (MJ1971kjf). The model component for the noise in the speed-density relationship is defined as independent
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      ...
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, (curr_k_crit/(k_jam - curr_k_crit))*((k_jam/V2) - 1.0), 1.0), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, (par1/(k_jam - par1))*((k_jam/V2) - 1.0), 1.0), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    if (model_obj$converged != TRUE) {
