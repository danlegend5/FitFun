fit_flow_density_with_GS1935kjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Greenshields model with fixed
#              jam density (GS1935kjf). The model component for the noise in the flow-density relationship is defined as independent observations
...
functional_form_model = 'GS1935kjf'
...
cat('  Greenshields\n')
cat('  Fixed jam density (GS1935kjf)\n')
cat('\n')
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - (V2/k_jam))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
...
tryCatch(
  { q_0 = 0.0
    v_ff = NA
    dvdk_0 = -model_obj$mu.coefficients[1]/k_jam
    k_crit = NA
    k_vmax = NA
    q_cap = NA
    v_max = NA
