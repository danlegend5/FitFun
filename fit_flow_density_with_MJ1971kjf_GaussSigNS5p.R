fit_flow_density_with_MJ1971kjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      ...
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, curr_k_crit*((k_jam - V2)/(k_jam - curr_k_crit)), V2), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, par1*((k_jam - V2)/(k_jam - par1)), V2), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
