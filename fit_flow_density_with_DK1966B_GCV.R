fit_flow_density_with_DK1966B_GCV = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Drake multi-regime model B
#              (DK1966B). The model component for the noise in the flow-density relationship is defined as independent observations that follow a
#              Gaussian distribution with constant variance (GCV).
...
#
par1_min = 0.0001      # Minimum acceptable value for the free parameter k_jam (must be positive)
par1_step = 0.0001     # Step size for the free parameter k_jam (must be positive)
par2_nsteps = 100      # Number of steps to be used for the initial profiling of the free parameter k_b (must be greater than or equal to 2)
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    par1_init = max(exp(-init_model_obj$mu.coefficients[1]/init_model_obj$mu.coefficients[2]), par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, V2*log(p[1]/V2), V2*log(p[1]/curr_k_b)), sigma.formula = ~ 1, family = NO()))
    for (i in 1:par2_nsteps) {
      curr_k_b = data.frame(curr_k_b = k_b_vec[i])
      attach(curr_k_b)
      attach(traffic_data)
      optim_obj = find.hyper(model = model_formula, parameters = c(par1_init), k = 0.0, steps = c(par1_step), lower = c(par1_min), maxit = 500)
    ...
    for (j in 1:par2_nrefine) {
      ...
      ind_hi = min(par2_nsteps, ind_min_gdev + 4)
      ...
      for (i in 1:curr_par2_nsteps) {
        ...
        attach(curr_k_b)
        attach(traffic_data)
        optim_obj = find.hyper(model = model_formula, parameters = c(curr_par1_vec[i]), k = 0.0, steps = c(par1_step), lower = c(par1_min), maxit = 500)
        if (optim_obj$convergence != 0) {
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, V2*log(par1/V2), V2*log(par1/par2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      if (par1 > par2) {
        v_bw = model_obj$mu.coefficients[1]
        k_jam = par1
        v_ff = v_bw*log(k_jam/par2)
        k_crit = max(k_jam/exp(1.0), par2)
        if (k_crit > par2) {
          q_cap = v_bw*k_jam/exp(1.0)
        } else {
          q_cap = v_ff*k_crit
        }
        dvdk_kjam = -v_bw/k_jam
      }
    } else if (model_obj$mu.coefficients[1] < 0.0) {
      if (par1 < par2) { v_ff = model_obj$mu.coefficients[1]*log(par1/par2) }
    } },
...
cat('  v_bw:     ', model_obj$mu.coefficients[1], '\n')
cat('  k_b:      ', par2, '\n')
cat('  k_jam:    ', par1, '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        model_obj$mu.coefficients[1], '           # v_bw\n',
        par2, '           # k_b\n',
        par1, '           # k_jam\n',
