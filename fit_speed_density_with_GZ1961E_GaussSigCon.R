fit_speed_density_with_GZ1961E_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model E (GZ1961E). The
...
tryCatch(
  ...
  { model_formula = quote(gamlss(V3 ~ 0 + I(sqrt(log(p[1]/V2))/p[1]), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(sqrt(log(par1/V2))/par1), sigma.formula = ~ 1, family = NO(), data = traffic_data)
