fit_flow_density_with_GZ1961Ekjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model E with fixed jam
#              density (GZ1961Ekjf). The model component for the noise in the flow-density relationship is defined as independent observations that
...
functional_form_model = 'GZ1961Ekjf'
...
cat('  Gazis\n')
cat('  Model E\n')
cat('  Fixed jam density (GZ1961Ekjf)\n')
cat('\n')
...
tryCatch(
  { if (k_jam < data_max_density) {
    ...
    model_obj = gamlss(V3 ~ 0 + I((V2/k_jam)*sqrt(log(k_jam/V2))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
