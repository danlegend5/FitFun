fit_flow_density_with_MN2008kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par1_min = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - ((V2/k_jam_use)^p[2]))/(1.0 + (p[1]*((V2/k_jam_use)^p[2]))))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - ((V2/k_jam)^par2))/(1.0 + (par1*((V2/k_jam)^par2))))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
