fit_flow_density_with_GZ1961Hkjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
functional_form_model = 'GZ1961Hkjf'
...
cat('  Gazis\n')
cat('  Model H\n')
cat('  Fixed jam density (GZ1961Hkjf)\n')
cat('\n')
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/k_jam_use))^(1.0/(1.0 - p[1])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    if (optim_obj$convergence != 0) {
      ...
      if (optim_obj$par[1] <= (par1_min + par1_step)) {
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/k_jam))^(1.0/(1.0 - par1)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
...
tryCatch(
  { q_0 = 0.0
    v_ff = NA
    dvdk_0 = model_obj$mu.coefficients[1]/(k_jam*(par1 - 1.0))
    k_crit = NA
    k_vmax = NA
    q_cap = NA
    v_max = NA
