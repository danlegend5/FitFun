fit_flow_density_with_GZ1961Gkjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model G with fixed jam
#              density (GZ1961Gkjf). The model component for the noise in the flow-density relationship is defined as independent observations that
#              follow a Skew Exponential Power Type III distribution. The density dependence of the log of the scale parameter and the log of the
#              skewness parameter is modelled using natural cubic splines with five and three effective free parameters, respectively. The density
...
functional_form_model = 'GZ1961Gkjf'
...
cat('  Gazis\n')
cat('  Model G\n')
cat('  Fixed jam density (GZ1961Gkjf)\n')
cat('\n')
...
tryCatch(
  ...
  { k_jam_use = data.frame(k_jam_use = k_jam)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam_use)^(p[1] - 1.0)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam)^(par1 - 1.0)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
