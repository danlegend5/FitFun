fit_flow_density_with_GZ1961Gkjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              follow a Gaussian distribution. The density dependence of the log of the standard deviation is modelled using natural cubic splines
#              with five effective free parameters (GaussSigNS5p).
...
tryCatch(
  ...
  { k_jam_use = data.frame(k_jam_use = k_jam)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam_use)^(p[1] - 1.0)))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam)^(par1 - 1.0)))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
