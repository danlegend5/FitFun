fit_flow_density_with_UW1961Bkjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(exp(p[1]*V2) - exp(p[1]*k_jam_use))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(exp(par1*V2) - exp(par1*k_jam))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
