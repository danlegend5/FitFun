fit_flow_density_with_GZ1961B_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + V2 + I(V2^1.5), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
...
cat('  v_ff/sqrt(k_jam):', -model_obj$mu.coefficients[2], '\n')
