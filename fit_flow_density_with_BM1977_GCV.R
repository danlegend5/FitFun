fit_flow_density_with_BM1977_GCV = function(data, ngrid, upper_density, output_files) {
...
#              "FitFun.R". The model component for the functional form of the flow-density relationship is the Boardman model (BM1977). The model
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = data) },
...
tryCatch(
  { reconstructed_model_fit = data.table(V2 = seq(from = 0.0, to = upper_density, length.out = ngrid))
    ...
    predicted_values_for_mu[2:ngrid] = predict(model_obj, what = 'mu', newdata = reconstructed_model_fit[2:ngrid], type = 'response', data = data)
    predicted_values_for_sigma = predict(model_obj, what = 'sigma', newdata = reconstructed_model_fit, type = 'response', data = data)
...
tryCatch(
  { q_0 = 0.0
    ...
    q_cap = NA      # Can be computed analytically - not yet implemented
    v_max = NA      # Can be computed analytically - not yet implemented
    k_jam = NA
    v_bw = NA
    dvdk_kjam = NA },
