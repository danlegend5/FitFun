fit_speed_density_with_GZ1961Gkjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model G with fixed jam
#              density (GZ1961Gkjf). The model component for the noise in the speed-density relationship is defined as independent observations that
...
tryCatch(
  ...
  { k_jam_use = data.frame(k_jam_use = k_jam)
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - ((V2/k_jam_use)^(p[1] - 1.0))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - ((V2/k_jam)^(par1 - 1.0))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
