fit_speed_density_with_GZ1961Ckjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model C with fixed jam
#              density (GZ1961Ckjf). The model component for the noise in the speed-density relationship is defined as independent observations that
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(1.0 - ((V2/k_jam)^2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
