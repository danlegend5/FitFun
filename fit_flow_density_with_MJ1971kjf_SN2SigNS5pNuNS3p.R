fit_flow_density_with_MJ1971kjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              with fixed jam density (MJ1971kjf). The model component for the noise in the flow-density relationship is defined as independent
#              observations that follow a Skew Normal Type II distribution. The density dependence of the log of the scale parameter and the log
#              of the skewness parameter is modelled using natural cubic splines with five and three effective free parameters, respectively
#              (SN2SigNS5pNuNS3p).
...
par1_nsteps = 50       # Number of steps to be used for the initial profiling of the free parameter k_crit (must be greater than or equal to 2)
par1_nrefine = 7       # Number of refinement iterations to be performed for fitting the free parameter k_crit (must be greater than or equal to 1)
...
cat('  Munjal\n')
cat('  Multi-regime\n')
cat('  Fixed jam density (MJ1971kjf)\n')
cat('\n')
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    k_crit_vec = seq(from = k_crit_lo, to = k_crit_hi, length.out = par1_nsteps + 2)
    k_crit_vec = k_crit_vec[2:(par1_nsteps + 1)]
    gdev_vec = double(length = par1_nsteps)
    for (i in 1:par1_nsteps) {
      curr_k_crit = k_crit_vec[i]
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, curr_k_crit*((k_jam - V2)/(k_jam - curr_k_crit)), V2), sigma.formula = ~ ns(V2, df = 4),
                         nu.formula = ~ ns(V2, df = 2), family = SN2(), data = traffic_data,
    ...
    for (j in 1:par1_nrefine) {
      ...
      curr_k_crit_vec = double(length = curr_par1_nsteps)
    ...
    par1 = k_crit_vec[ind_min_gdev]

    # Perform the final fit
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, par1*((k_jam - V2)/(k_jam - par1)), V2), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
                       family = SN2(), data = traffic_data, c.crit = outer_ccrit, n.cyc = outer_ncyc, i.control = glim.control(cc = inner_ccrit, cyc = inner_ncyc))
    if (model_obj$converged != TRUE) {
