fit_speed_density_with_MN2008_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the MacNicholas model (MN2008).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + log(V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    par1_min = 0.0
    model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - ((V2/p[3])^p[2]))/(1.0 + (p[1]*((V2/p[3])^p[2])))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - ((V2/(par3_min_use + abs(p[3])))^(par2_min_use + abs(p[2]))))/(1.0 + (abs(p[1])*((V2/(par3_min_use + abs(p[3])))^(par2_min_use + abs(p[2])))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I((1.0 - ((V2/par3)^par2))/(1.0 + (par1*((V2/par3)^par2)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
