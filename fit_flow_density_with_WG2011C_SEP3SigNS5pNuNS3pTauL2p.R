fit_flow_density_with_WG2011C_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par2_init = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2/(1.0 + exp(p[1]*(V2 - p[2])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
                                 family = SEP3(), c.crit = outer_ccrit_use, n.cyc = outer_ncyc_use, i.control = glim.control(cc = inner_ccrit_use, cyc = inner_ncyc_use)))
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2/(1.0 + exp(par1*(V2 - par2)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
cat('  c_2:  ', model_obj$mu.coefficients[1], '\n')
cat('  c_3:  ', par1, '\n')
cat('  k_ref:', par2, '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        model_obj$mu.coefficients[1], '           # c_2\n',
        par1, '           # c_3\n',
        par2, '           # k_ref\n',
