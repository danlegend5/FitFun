fit_flow_density_with_GZ1961Ckjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model C with fixed jam
#              density (GZ1961Ckjf). The model component for the noise in the flow-density relationship is defined as independent observations that
...
functional_form_model = 'GZ1961Ckjf'
...
cat('  Gazis\n')
cat('  Model C\n')
cat('  Fixed jam density (GZ1961Ckjf)\n')
cat('\n')
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam)^2))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
