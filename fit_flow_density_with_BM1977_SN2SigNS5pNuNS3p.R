fit_flow_density_with_BM1977_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2 + I(V2^2), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(mu.link = 'log'),
