fit_flow_density_with_BD1995_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Bando model (BD1995). The
...
#
par1_min = 0.0001      # Minimum acceptable value for the free parameter c_1 (must be positive)
par1_step = 0.0001     # Step size for the free parameter c_1 (must be positive)
par2_step = 0.0001     # Step size for the free parameter c_2 (must be positive)
...
tryCatch(
  ...
  { par1_init = max(data_max_density, par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((tanh((p[1]/V2) - p[2]) + tanh(p[2]))/(1.0 + tanh(p[2])))), sigma.formula = ~ ns(V2, df = 4),
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(V2*((tanh(((par1_min_use + abs(p[1]))/V2) - p[2]) + tanh(p[2]))/(1.0 + tanh(p[2])))), sigma.formula = ~ ns(V2, df = 4),
                                   nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(), c.crit = outer_ccrit_use, n.cyc = outer_ncyc_use,
                                   i.control = glim.control(cc = inner_ccrit_use, cyc = inner_ncyc_use)))
      attach(par1_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((tanh((par1/V2) - par2) + tanh(par2))/(1.0 + tanh(par2)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
