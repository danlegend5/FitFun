fit_flow_density_with_DK1966B_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              Skew Exponential Power Type III distribution. The density dependence of the log of the scale parameter and the log of the skewness
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, V2*log(par1/V2), V2*log(par1/par2)), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
