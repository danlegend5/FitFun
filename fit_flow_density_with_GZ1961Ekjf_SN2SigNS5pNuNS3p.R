fit_flow_density_with_GZ1961Ekjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  { if (k_jam < data_max_density) {
    ...
    model_obj = gamlss(V3 ~ 0 + I((V2/k_jam)*sqrt(log(k_jam/V2))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
