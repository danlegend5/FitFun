fit_flow_density_with_DC2012Bkjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam <= data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + ((p[1]*((1.0/V2) - (1.0/k_jam_use)))^(-p[2])))^(-1.0/p[2]))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 + ((par1*((1.0/V2) - (1.0/k_jam)))^(-par2)))^(-1.0/par2))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
