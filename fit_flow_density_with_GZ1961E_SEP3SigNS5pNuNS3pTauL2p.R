fit_flow_density_with_GZ1961E_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model E (GZ1961E). The
...
tryCatch(
  ...
  { inner_ccrit_use = data.frame(inner_ccrit_use = inner_ccrit)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I((V2/p[1])*sqrt(log(p[1]/V2))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    model_obj = gamlss(V3 ~ 0 + I((V2/par1)*sqrt(log(par1/V2))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      q_cap = model_obj$mu.coefficients[1]/sqrt(2.0*exp(1.0))
      k_jam = par1
      k_crit = exp(-0.5)*k_jam
    } },
...
cat('  q_cap:', model_obj$mu.coefficients[1]/sqrt(2.0*exp(1.0)), '\n')
cat('  k_jam:', par1, '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        model_obj$mu.coefficients[1]/sqrt(2.0*exp(1.0)), '           # q_cap\n',
        par1, '           # k_jam\n',
