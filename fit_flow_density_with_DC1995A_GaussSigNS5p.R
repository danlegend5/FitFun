fit_flow_density_with_DC1995A_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Del Castillo model A (DC1995A).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (p[1]*((1.0/V2) - (1.0/p[2]))))^p[3])))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (par1*((1.0/V2) - (1.0/par2))))^par3)))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
