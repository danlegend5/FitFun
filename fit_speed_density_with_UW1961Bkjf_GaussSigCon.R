fit_speed_density_with_UW1961Bkjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Underwood model B with fixed
#              jam density (UW1961Bkjf). The model component for the noise in the speed-density relationship is defined as independent observations
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(exp(p[1]*V2) - exp(p[1]*k_jam_use)), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(exp(par1*V2) - exp(par1*k_jam)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
