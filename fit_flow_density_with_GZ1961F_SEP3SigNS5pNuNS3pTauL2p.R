fit_flow_density_with_GZ1961F_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model F (GZ1961F). The
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + I(V2^2), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[2] < 0.0) {
      k_crit = sqrt(-0.5/model_obj$mu.coefficients[2])
      k_vmax = 0.0
      q_cap = v_ff*k_crit*exp(-0.5)
      v_max = v_ff
    } },
...
cat('  1/(k_crit^2):', -2.0*model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        -2.0*model_obj$mu.coefficients[2], '           # 1/(k_crit^2)\n',
