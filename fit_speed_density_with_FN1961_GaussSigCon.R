fit_speed_density_with_FN1961_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Franklin-Newell model (FN1961).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - (p[1]^((1.0/V2) - (1.0/p[2])))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - (max(min(p[1], par1_max_use), par1_min_use)^((1.0/V2) - (1.0/(par2_min_use + abs(p[2])))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - (par1^((1.0/V2) - (1.0/par2)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
