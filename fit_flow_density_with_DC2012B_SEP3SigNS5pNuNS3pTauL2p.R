fit_flow_density_with_DC2012B_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Del Castillo model B (DC2012B).
...
#              parameter is modelled using natural cubic splines with five and three effective free parameters, respectively. The density dependence
#              of the log of the kurtosis parameter is modelled using a straight line function (i.e. an intercept and a gradient as the two free
#              parameters; SEP3SigNS5pNuNS3pTauL2p).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    # Fit a Greenberg model to estimate initial values for k_jam and (v_bw*k_jam)/v_ff
    ...
    par2_min = data_max_density + par2_step
    par2_init = max(exp(-init_model_obj$mu.coefficients[1]/init_model_obj$mu.coefficients[2]), par2_min + par2_step)
    par1_init = max((-init_model_obj$mu.coefficients[2]*par2_init)/v_ff_init, par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + ((p[1]*((1.0/V2) - (1.0/p[2])))^(-p[3])))^(-1.0/p[3]))), sigma.formula = ~ ns(V2, df = 4),
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + (((par1_min_use + abs(p[1]))*((1.0/V2) - (1.0/(par2_min_use + abs(p[2])))))^(-(par3_min_use + abs(p[3])))))^(-1.0/(par3_min_use + abs(p[3]))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 + ((par1*((1.0/V2) - (1.0/par2)))^(-par3)))^(-1.0/par3))), sigma.formula = ~ ns(V2, df = 4),
                       nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(), data = traffic_data,
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      k_jam = par2
      v_bw = par1*(v_ff/k_jam)
      k_crit = k_jam/(1.0 + ((v_ff/v_bw)^(par3/(par3 + 1.0))))
      k_vmax = 0.0
      q_cap = v_ff*k_crit*((k_jam/k_crit)^(-1.0/par3))
      v_max = v_ff
