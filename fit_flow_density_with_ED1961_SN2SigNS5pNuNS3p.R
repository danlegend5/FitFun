...
cat('  1/k_crit:      ', sprintf('%.8g', -par1), '\n')
cat('  k_b:           ', sprintf('%.8g', par2), '\n')
cat('  v_bw*ln(k_jam):', sprintf('%.8g', model_obj$mu.coefficients[2]), '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        sprintf('%.8g', -par1), '           # 1/k_crit\n',
        sprintf('%.8g', par2), '           # k_b\n',
        sprintf('%.8g', model_obj$mu.coefficients[2]), '           # v_bw*ln(k_jam)\n',
