fit_flow_density_with_ED1961_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Edie multi-regime model
#              (ED1961). The model component for the noise in the flow-density relationship is defined as independent observations that follow a
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, 0.0, V2*exp(p[1]*V2)) + ifelse(V2 > curr_k_b, V2, 0.0) + ifelse(V2 > curr_k_b, V2*log(V2), 0.0),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, 0.0, V2*exp(par1*V2)) + ifelse(V2 > par2, V2, 0.0) + ifelse(V2 > par2, V2*log(V2), 0.0),
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[3] < 0.0) {
      ...
      k_jam = exp(model_obj$mu.coefficients[2]/v_bw)
...
cat('  1/k_crit:      ', -par1, '\n')
cat('  k_b:           ', par2, '\n')
cat('  v_bw*ln(k_jam):', model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        -par1, '           # 1/k_crit\n',
        par2, '           # k_b\n',
        model_obj$mu.coefficients[2], '           # v_bw*ln(k_jam)\n',
