fit_speed_density_with_ED1961kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Edie multi-regime model with
#              fixed jam density (ED1961kjf). The model component for the noise in the speed-density relationship is defined as independent
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, 0.0, exp(p[1]*V2)) + ifelse(V2 > curr_k_b, log(k_jam_use/V2), 0.0),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, 0.0, exp(par1*V2)) + ifelse(V2 > par2, log(k_jam/V2), 0.0),
