fit_flow_density_with_DK1966B_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, V2*log(p[1]/V2), V2*log(p[1]/curr_k_b)), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, V2*log(par1/V2), V2*log(par1/par2)), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
