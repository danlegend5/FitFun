fit_flow_density_with_FN1961kjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Franklin-Newell model with
#              fixed jam density (FN1961kjf). The model component for the noise in the flow-density relationship is defined as independent
...
functional_form_model = 'FN1961kjf'
...
cat('  Franklin-Newell\n')
cat('  Fixed jam density (FN1961kjf)\n')
cat('\n')
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - (V2/k_jam))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - (p[1]^((1.0/V2) - (1.0/k_jam_use))))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
                                 family = SN2(), c.crit = outer_ccrit_use, n.cyc = outer_ncyc_use, i.control = glim.control(cc = inner_ccrit_use, cyc = inner_ncyc_use)))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    optim_obj = try(find.hyper(model = model_formula, parameters = c(par1_init), k = 0.0, steps = c(par1_step), lower = c(par1_min), upper = c(par1_max), maxit = 500))
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - (par1^((1.0/V2) - (1.0/k_jam))))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
