fit_speed_density_with_FN1961kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Franklin-Newell model with
#              fixed jam density (FN1961kjf). The model component for the noise in the speed-density relationship is defined as independent
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + I(1.0 - (V2/k_jam)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - (p[1]^((1.0/V2) - (1.0/k_jam_use)))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - (par1^((1.0/V2) - (1.0/k_jam)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
