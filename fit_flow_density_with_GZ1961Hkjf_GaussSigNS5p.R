fit_flow_density_with_GZ1961Hkjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/k_jam_use))^(1.0/(1.0 - p[1])))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/k_jam))^(1.0/(1.0 - par1)))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
