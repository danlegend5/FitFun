fit_flow_density_with_DC2012Bkjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Del Castillo model B with
#              fixed jam density (DC2012Bkjf). The model component for the noise in the flow-density relationship is defined as independent
...
functional_form_model = 'DC2012Bkjf'
...
cat('  Del Castillo\n')
cat('  Model B\n')
cat('  Fixed jam density (DC2012Bkjf)\n')
cat('\n')
...
tryCatch(
  ...
  { if (k_jam <= data_max_density) {
    ...
    # Fit a Greenberg model with fixed jam density to estimate an initial value for (v_bw*k_jam)/v_ff
    ...
    par1_init = max((init_model_obj$mu.coefficients[1]*k_jam)/v_ff_init, par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + ((p[1]*((1.0/V2) - (1.0/k_jam_use)))^(-p[2])))^(-1.0/p[2]))), sigma.formula = ~ ns(V2, df = 4),
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + (((par1_min_use + abs(p[1]))*((1.0/V2) - (1.0/k_jam_use)))^(-(par2_min_use + abs(p[2])))))^(-1.0/(par2_min_use + abs(p[2]))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 + ((par1*((1.0/V2) - (1.0/k_jam)))^(-par2)))^(-1.0/par2))), sigma.formula = ~ ns(V2, df = 4),
                       nu.formula = ~ ns(V2, df = 2), family = SN2(), data = traffic_data, c.crit = outer_ccrit, n.cyc = outer_ncyc,
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      k_crit = k_jam/(1.0 + ((v_ff/v_bw)^(par2/(par2 + 1.0))))
      k_vmax = 0.0
      q_cap = v_ff*k_crit*((k_jam/k_crit)^(-1.0/par2))
      v_max = v_ff
