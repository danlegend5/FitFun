fit_flow_density_with_GD2008kjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par1_min = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*log((p[1] + k_jam_use)/(p[1] + V2))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*log((par1 + k_jam)/(par1 + V2))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
