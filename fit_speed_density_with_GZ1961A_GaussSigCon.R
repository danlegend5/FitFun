fit_speed_density_with_GZ1961A_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model A (GZ1961A). The
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + I(1.0/sqrt(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] < 0.0) {
      ...
      k_jam = (model_obj$mu.coefficients[2]/model_obj$mu.coefficients[1])^2
      k_crit = 0.25*k_jam
...
cat('  v_bw*sqrt(k_jam):', 0.5*model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Speed.Density', ntraffic_data, data_min_density, data_max_density, data_min_speed, data_max_speed,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        0.5*model_obj$mu.coefficients[2], '           # v_bw*sqrt(k_jam)\n',
