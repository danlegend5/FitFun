fit_flow_density_with_ED1961kjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, 0.0, V2*exp(par1*V2)) + ifelse(V2 > par2, V2*log(k_jam/V2), 0.0), sigma.formula = ~ ns(V2, df = 4),
                       family = NO(), data = traffic_data, c.crit = outer_ccrit, n.cyc = outer_ncyc, i.control = glim.control(cc = inner_ccrit, cyc = inner_ncyc))
