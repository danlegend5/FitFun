fit_flow_density_with_WG2011A_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par2_init = 0.0
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + V2 + I(V2*((1.0 + exp(p[1]*(V2 - p[2])))^(-(par3_min_use + abs(p[3]))))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
                                   family = SN2(), c.crit = outer_ccrit_use, n.cyc = outer_ncyc_use, i.control = glim.control(cc = inner_ccrit_use, cyc = inner_ncyc_use)))
      attach(par3_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + V2 + I(V2*((1.0 + exp(par1*(V2 - par2)))^(-par3))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
