fit_speed_density_with_DK1966Akjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              with fixed jam density (DK1966Akjf). The model component for the noise in the speed-density relationship is defined as independent
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      curr_k_b = k_b_vec[i]
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, 0.0, 1.0) + ifelse(V2 > curr_k_b, 0.0, V2) + ifelse(V2 > curr_k_b, 1.0 - (V2/k_jam), 0.0),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, 0.0, 1.0) + ifelse(V2 > par1, 0.0, V2) + ifelse(V2 > par1, 1.0 - (V2/k_jam), 0.0),
                       sigma.formula = ~ 1, family = NO(), data = traffic_data)
    if (model_obj$converged != TRUE) {
