fit_flow_density_with_DC1995Akjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (p[1]*((1.0/V2) - (1.0/k_jam_use))))^p[2])))), sigma.formula = ~ ns(V2, df = 4),
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + ((par1_min_use + abs(p[1]))*((1.0/V2) - (1.0/k_jam_use))))^(par2_min_use + abs(p[2])))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (par1*((1.0/V2) - (1.0/k_jam))))^par2)))), sigma.formula = ~ ns(V2, df = 4),
