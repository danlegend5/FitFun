fit_flow_density_with_DK1966Bkjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      curr_k_b = k_b_vec[i]
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, V2*log(k_jam/V2), V2*log(k_jam/curr_k_b)), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, V2*log(k_jam/V2), V2*log(k_jam/par1)), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
                       c.crit = outer_ccrit, n.cyc = outer_ncyc, i.control = glim.control(cc = inner_ccrit, cyc = inner_ncyc))
    if (model_obj$converged != TRUE) {
