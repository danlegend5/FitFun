fit_flow_density_with_UW1961B_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*exp(p[1]*V2)) + V2, sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*exp(par1*V2)) + V2, sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
