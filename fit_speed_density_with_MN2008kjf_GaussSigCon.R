fit_speed_density_with_MN2008kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the MacNicholas model with fixed
#              jam density (MN2008kjf). The model component for the noise in the speed-density relationship is defined as independent observations
...
tryCatch(
  ...
  { par1_min = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - ((V2/k_jam_use)^p[2]))/(1.0 + (p[1]*((V2/k_jam_use)^p[2])))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - ((V2/k_jam_use)^(par2_min_use + abs(p[2]))))/(1.0 + (abs(p[1])*((V2/k_jam_use)^(par2_min_use + abs(p[2])))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I((1.0 - ((V2/k_jam)^par2))/(1.0 + (par1*((V2/k_jam)^par2)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
