fit_flow_density_with_GZ1961Akjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model A with fixed jam
#              density (GZ1961Akjf). The model component for the noise in the flow-density relationship is defined as independent observations that
...
functional_form_model = 'GZ1961Akjf'
...
cat('  Gazis\n')
cat('  Model A\n')
cat('  Fixed jam density (GZ1961Akjf)\n')
cat('\n')
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(sqrt(k_jam*V2) - V2), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      v_bw = 0.5*model_obj$mu.coefficients[1]
      k_crit = 0.25*k_jam
...
cat('  v_bw:', 0.5*model_obj$mu.coefficients[1], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        0.5*model_obj$mu.coefficients[1], '           # v_bw\n',
