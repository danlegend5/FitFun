fit_flow_density_with_GZ1961Bkjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model B with fixed jam
#              density (GZ1961Bkjf). The model component for the noise in the flow-density relationship is defined as independent observations that
...
functional_form_model = 'GZ1961Bkjf'
...
cat('  Gazis\n')
cat('  Model B\n')
cat('  Fixed jam density (GZ1961Bkjf)\n')
cat('\n')
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - sqrt(V2/k_jam))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
