fit_flow_density_with_GZ1961Gkjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { k_jam_use = data.frame(k_jam_use = k_jam)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam_use)^(p[1] - 1.0)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - ((V2/k_jam)^(par1 - 1.0)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
