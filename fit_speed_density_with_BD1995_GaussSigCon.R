fit_speed_density_with_BD1995_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Bando model (BD1995). The
...
tryCatch(
  ...
  { par1_init = max(data_max_density, par1_min + par1_step)
    model_formula = quote(gamlss(V3 ~ 0 + I((tanh((p[1]/V2) - p[2]) + tanh(p[2]))/(1.0 + tanh(p[2]))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I((tanh(((par1_min_use + abs(p[1]))/V2) - p[2]) + tanh(p[2]))/(1.0 + tanh(p[2]))), sigma.formula = ~ 1, family = NO()))
      attach(par1_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + I((tanh((par1/V2) - par2) + tanh(par2))/(1.0 + tanh(par2))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
