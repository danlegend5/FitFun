fit_flow_density_with_GZ1961D_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { inner_ccrit_use = data.frame(inner_ccrit_use = inner_ccrit)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(sqrt((V2/p[1])*(1.0 - (V2/p[1])))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(sqrt((V2/par1)*(1.0 - (V2/par1)))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
