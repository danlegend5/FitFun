fit_flow_density_with_MJ1971_GCV = function(traffic_data, ngrid, upper_density, output_files) {
...
par2_nsteps = 100      # Number of steps to be used for the initial profiling of the free parameter k_crit (must be greater than or equal to 2)
...
tryCatch(
  ...
  { par1_init = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, p[1]*(curr_k_crit - V2) + curr_k_crit, V2), sigma.formula = ~ 1, family = NO()))
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, par1*(par2 - V2) + par2, V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
