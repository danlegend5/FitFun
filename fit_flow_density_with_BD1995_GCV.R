fit_flow_density_with_BD1995_GCV = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par1_init = max(data_max_density, par1_min + par1_step)
    ...
    if (optim_obj$convergence != 0) {
      optim_obj = find.hyper(model = model_formula, parameters = c(par1_init, 0.0), k = 0.0, steps = c(par1_step, par2_step), method = 'Nelder-Mead', maxit = 500)
      if (optim_obj$convergence != 0) {
        cat('ERROR - The intermediate fits did not converge...\n')
        detach(traffic_data)
        q(save = 'no', status = 1)
      }
      if (optim_obj$par[1] < par1_min) {
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((tanh((par1/V2) - par2) + tanh(par2))/(1.0 + tanh(par2)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
