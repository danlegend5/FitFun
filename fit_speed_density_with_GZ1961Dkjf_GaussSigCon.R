fit_speed_density_with_GZ1961Dkjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model D with fixed jam
#              density (GZ1961Dkjf). The model component for the noise in the speed-density relationship is defined as independent observations that
...
tryCatch(
  { if (k_jam < data_max_density) {
    ...
    model_obj = gamlss(V3 ~ 0 + I(sqrt((1.0 - (V2/k_jam))/(V2*k_jam))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
