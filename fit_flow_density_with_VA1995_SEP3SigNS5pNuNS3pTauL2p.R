fit_flow_density_with_VA1995_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - (p[1]*V2) - sqrt((((p[2]*V2) - 1.0)^2) + (p[3]*(V2^2)))), sigma.formula = ~ ns(V2, df = 4),
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - (beta*V2) - sqrt((((gamma*V2) - 1.0)^2) + (delta*(V2^2)))), sigma.formula = ~ ns(V2, df = 4),
...
cat('  alpha:', sprintf('%.8g', model_obj$mu.coefficients[1]), '\n')
cat('  beta: ', sprintf('%.8g', beta), '\n')
cat('  gamma:', sprintf('%.8g', gamma), '\n')
cat('  delta:', sprintf('%.8g', delta), '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        sprintf('%.8g', model_obj$mu.coefficients[1]), '           # alpha\n',
        sprintf('%.8g', beta), '           # beta\n',
        sprintf('%.8g', gamma), '           # gamma\n',
        sprintf('%.8g', delta), '           # delta\n',
