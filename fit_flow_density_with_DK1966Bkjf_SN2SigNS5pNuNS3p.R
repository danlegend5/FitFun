fit_flow_density_with_DK1966Bkjf_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              with fixed jam density (DK1966Bkjf). The model component for the noise in the flow-density relationship is defined as independent
...
functional_form_model = 'DK1966Bkjf'
...
cat('  Multi-regime\n')
cat('  Model B\n')
cat('  Fixed jam density (DK1966Bkjf)\n')
cat('\n')
...
tryCatch(
  ...
  { if (k_jam < data_max_density) {
    ...
    for (i in 1:par1_nsteps) {
      curr_k_b = k_b_vec[i]
      model_obj = gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, V2*log(k_jam/V2), V2*log(k_jam/curr_k_b)), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par1, V2*log(k_jam/V2), V2*log(k_jam/par1)), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2),
                       family = SN2(), data = traffic_data, c.crit = outer_ccrit, n.cyc = outer_ncyc, i.control = glim.control(cc = inner_ccrit, cyc = inner_ncyc))
    if (model_obj$converged != TRUE) {
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      v_ff = v_bw*log(k_jam/par1)
      k_crit = max(k_jam/exp(1.0), par1)
      if (k_crit > par1) {
