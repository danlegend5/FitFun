fit_flow_density_with_VA1995_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - (p[1]*V2) - sqrt((((p[2]*V2) - 1.0)^2) + (p[3]*(V2^2)))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - (beta*V2) - sqrt((((gamma*V2) - 1.0)^2) + (delta*(V2^2)))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
