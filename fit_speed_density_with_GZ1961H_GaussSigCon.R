fit_speed_density_with_GZ1961H_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Gazis model H (GZ1961H). The
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + log(V2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - (V2/p[2]))^(1.0/(1.0 - p[1]))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I((1.0 - (V2/(par2_min_use + abs(p[2]))))^(1.0/(1.0 - (par1_max_use - abs(p[1]))))), sigma.formula = ~ 1, family = NO()))
      attach(par1_max_use)
      attach(par2_min_use)
    ...
    model_obj = gamlss(V3 ~ 0 + I((1.0 - (V2/par2))^(1.0/(1.0 - par1))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
