fit_flow_density_with_UW1961B_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Underwood model B (UW1961B).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*exp(p[1]*V2)) + V2, sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*exp(par1*V2)) + V2, sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
...
tryCatch(
  { q_0 = 0.0
    ...
    k_jam = NA         # Can be computed analytically - not yet implemented
    v_bw = NA          # Can be computed analytically - not yet implemented
    dvdk_kjam = NA     # Can be computed analytically - not yet implemented
    tmp_val1 = model_obj$mu.coefficients[1] + model_obj$mu.coefficients[2]
...
cat('  v_ff (non-physical):    ', sprintf('%.8g', model_obj$mu.coefficients[1]), '\n')
cat('  1/k_crit (non-physical):', sprintf('%.8g', -par1), '\n')
cat('  a:                      ', sprintf('%.8g', -model_obj$mu.coefficients[2]), '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        sprintf('%.8g', model_obj$mu.coefficients[1]), '           # v_ff (non-physical)\n',
        sprintf('%.8g', -par1), '           # 1/k_crit (non-physical)\n',
        sprintf('%.8g', -model_obj$mu.coefficients[2]), '           # a\n',
