fit_flow_density_with_UW1961A_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Underwood model A (UW1961A).
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
...
cat('  1/k_crit:', -model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        -model_obj$mu.coefficients[2], '           # 1/k_crit\n',
