fit_speed_density_with_GS1935kjf_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Greenshields model with fixed
#              jam density (GS1935kjf). The model component for the noise in the speed-density relationship is defined as independent observations
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(1.0 - (V2/k_jam)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
