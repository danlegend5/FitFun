fit_speed_density_with_WG2011A_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Wang model A (WG2011A). The
...
tryCatch(
  ...
  { par2_init = 0.0
    model_formula = quote(gamlss(V3 ~ 1 + I((1.0 + exp(p[1]*(V2 - p[2])))^(-p[3])), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 1 + I((1.0 + exp(p[1]*(V2 - p[2])))^(-(par3_min_use + abs(p[3])))), sigma.formula = ~ 1, family = NO()))
      attach(par3_min_use)
    ...
    model_obj = gamlss(V3 ~ 1 + I((1.0 + exp(par1*(V2 - par2)))^(-par3)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
