fit_flow_density_with_GZ1961H_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              model component for the noise in the flow-density relationship is defined as independent observations that follow a Skew Normal Type
#              II distribution. The density dependence of the log of the scale parameter and the log of the skewness parameter is modelled using
#              natural cubic splines with five and three effective free parameters, respectively (SN2SigNS5pNuNS3p).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/p[2]))^(1.0/(1.0 - p[1])))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 - (V2/par2))^(1.0/(1.0 - par1)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
