fit_speed_density_with_ED1961_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Edie multi-regime model
#              (ED1961). The model component for the noise in the speed-density relationship is defined as independent observations that follow a
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_b, 0.0, exp(p[1]*V2)) + ifelse(V2 > curr_k_b, 1.0, 0.0) + ifelse(V2 > curr_k_b, log(V2), 0.0),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, 0.0, exp(par1*V2)) + ifelse(V2 > par2, 1.0, 0.0) + ifelse(V2 > par2, log(V2), 0.0),
