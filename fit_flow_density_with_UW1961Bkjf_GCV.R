fit_flow_density_with_UW1961Bkjf_GCV = function(data, ngrid, upper_density, output_files) {
...
#              "FitFun.R". The model component for the functional form of the flow-density relationship is the Underwood model B with fixed jam
#              density (UW1961Bkjf). The model component for the noise in the flow-density relationship is defined as independent observations that
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + offset(log(V2)) + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = data)
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((exp(V2)^par1) - (exp(k_jam)^par1))), sigma.formula = ~ 1, family = NO(), data = data)
    ...
    model_obj$sbc = model_obj$sbc + log(ndata) },
...
tryCatch(
  { q_0 = 0.0
    ...
    dvdk_kjam = NA
    tmp_val1 = par1*k_jam
    tmp_val2 = exp(tmp_val1)
    tmp_val3 = model_obj$mu.coefficients[1]*(1.0 - tmp_val2)
    if (tmp_val3 > 0.0) {
      v_ff = tmp_val3
      if (par1 < 0.0) {
        k_vmax = 0.0
