fit_speed_density_with_UW1961A_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Underwood model A (UW1961A).
...
tryCatch(
  { model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
