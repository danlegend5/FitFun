fit_flow_density_with_DC1995A_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Del Castillo model A
#              (DC1995A). The model component for the noise in the flow-density relationship is defined as independent observations that follow a
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    # Fit a Greenberg model to estimate initial values for k_jam and (v_bw*k_jam)/(m*v_ff)
    ...
    par1_init = max((-init_model_obj$mu.coefficients[2]*par2_init)/(par3_init*v_ff_init), par1_min + par1_step)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (p[1]*((1.0/V2) - (1.0/p[2]))))^p[3])))), sigma.formula = ~ ns(V2, df = 4),
    ...
    if (optim_obj$convergence != 0) {
      ...
      attach(dmd_use)
      attach(par3_min_use)
      optim_obj = try(find.hyper(model = model_formula, parameters = c(par1_init - par1_min, par2_init - data_max_density, par3_init - par3_min), k = 0.0,
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*(1.0 - exp(1.0 - ((1.0 + (par1*((1.0/V2) - (1.0/par2))))^par3)))), sigma.formula = ~ ns(V2, df = 4),
