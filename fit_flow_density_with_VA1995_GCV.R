...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2^2), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    # Compute initial values for the Van Aerde parameterisation c1, c2, and c3
    c1_init = 0.0
    c2_init = v_ff_init/k_jam_init
    c3_init = max(((exp(1.0)/v_bw_init) - (4.0/v_ff_init))/k_jam_init, 0.0)

    # Compute initial values for the Bramich parameterisation beta, gamma, and delta
  ...
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      if (gamma > beta) {
        ...
        tmp_val = (gamma*gamma) - (beta*beta) + delta
        if (tmp_val > 0.0) {
