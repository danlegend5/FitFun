fit_flow_density_with_MJ1971_SN2SigNS5pNuNS3p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par1_init = 0.0
    ...
    model_formula = quote(gamlss(V3 ~ 0 + ifelse(V2 > curr_k_crit, p[1]*(curr_k_crit - V2) + curr_k_crit, V2), sigma.formula = ~ ns(V2, df = 4),
    ...
    model_obj = gamlss(V3 ~ 0 + ifelse(V2 > par2, par1*(par2 - V2) + par2, V2), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), family = SN2(),
