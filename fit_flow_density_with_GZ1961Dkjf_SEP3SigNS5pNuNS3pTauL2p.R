fit_flow_density_with_GZ1961Dkjf_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  { if (k_jam < data_max_density) {
    ...
    model_obj = gamlss(V3 ~ 0 + I(sqrt((V2/k_jam)*(1.0 - (V2/k_jam)))), sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2,
