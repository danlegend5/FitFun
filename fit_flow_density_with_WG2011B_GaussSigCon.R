fit_flow_density_with_WG2011B_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { par2_init = 0.0
    model_formula = quote(gamlss(V3 ~ 0 + V2 + I(V2/(1.0 + exp(p[1]*(V2 - p[2])))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 0 + V2 + I(V2/(1.0 + exp(par1*(V2 - par2)))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
