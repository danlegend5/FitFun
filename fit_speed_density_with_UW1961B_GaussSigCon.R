fit_speed_density_with_UW1961B_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Underwood model B (UW1961B).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(mu.link = 'log'), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 1 + I(exp(p[1]*V2)), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    model_obj = gamlss(V3 ~ 1 + I(exp(par1*V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { q_0 = 0.0
    v_ff = NA
    dvdk_0 = par1*model_obj$mu.coefficients[2]
...
cat('  v_ff (non-physical):    ', model_obj$mu.coefficients[2], '\n')
cat('  1/k_crit (non-physical):', -par1, '\n')
cat('  a:                      ', -model_obj$mu.coefficients[1], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Speed.Density', ntraffic_data, data_min_density, data_max_density, data_min_speed, data_max_speed,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        model_obj$mu.coefficients[2], '           # v_ff (non-physical)\n',
        -par1, '           # 1/k_crit (non-physical)\n',
        -model_obj$mu.coefficients[1], '           # a\n',
