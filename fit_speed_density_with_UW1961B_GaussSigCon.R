...
cat('  v_ff (non-physical):    ', sprintf('%.8g', model_obj$mu.coefficients[2]), '\n')
cat('  1/k_crit (non-physical):', sprintf('%.8g', -par1), '\n')
cat('  a:                      ', sprintf('%.8g', -model_obj$mu.coefficients[1]), '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Speed.Density', ntraffic_data, data_min_density, data_max_density, data_min_speed, data_max_speed,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        sprintf('%.8g', model_obj$mu.coefficients[2]), '           # v_ff (non-physical)\n',
        sprintf('%.8g', -par1), '           # 1/k_crit (non-physical)\n',
        sprintf('%.8g', -model_obj$mu.coefficients[1]), '           # a\n',
