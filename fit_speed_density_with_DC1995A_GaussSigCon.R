fit_speed_density_with_DC1995A_GaussSigCon = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the speed-density relationship is the Del Castillo model A (DC1995A).
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 1 + V2, sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - exp(1.0 - ((1.0 + (p[1]*((1.0/V2) - (1.0/p[2]))))^p[3]))), sigma.formula = ~ 1, family = NO()))
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      model_formula = quote(gamlss(V3 ~ 0 + I(1.0 - exp(1.0 - ((1.0 + ((par1_min_use + abs(p[1]))*((1.0/V2) - (1.0/(dmd_use + abs(p[2]))))))^(par3_min_use + abs(p[3]))))),
    ...
    model_obj = gamlss(V3 ~ 0 + I(1.0 - exp(1.0 - ((1.0 + (par1*((1.0/V2) - (1.0/par2))))^par3))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
