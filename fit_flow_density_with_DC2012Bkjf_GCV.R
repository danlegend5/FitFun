fit_flow_density_with_DC2012Bkjf_GCV = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { if (k_jam <= data_max_density) {
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*((1.0 + ((p[1]*((1.0/V2) - (1.0/k_jam_use)))^(-p[2])))^(-1.0/p[2]))), sigma.formula = ~ 1, family = NO()))
    attach(k_jam_use)
    attach(traffic_data)
    ...
    if (optim_obj$convergence != 0) {
      ...
      attach(par1_min_use)
      attach(par2_min_use)
      optim_obj = find.hyper(model = model_formula, parameters = c(par1_init - par1_min, par2_init - par2_min), k = 0.0, steps = c(par1_step, par2_step),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*((1.0 + ((par1*((1.0/V2) - (1.0/k_jam)))^(-par2)))^(-1.0/par2))), sigma.formula = ~ 1, family = NO(), data = traffic_data)
...
tryCatch(
  { q_0 = 0.0
    ...
    if (model_obj$mu.coefficients[1] > 0.0) {
      ...
      k_vmax = 0.0
      q_cap = v_ff*k_crit*((1.0 + ((par1*((1.0/k_crit) - (1.0/k_jam)))^(-par2)))^(-1.0/par2))
      v_max = v_ff
