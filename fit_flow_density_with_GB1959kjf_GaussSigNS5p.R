fit_flow_density_with_GB1959kjf_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Greenberg model with fixed
#              jam density (GB1959kjf). The model component for the noise in the flow-density relationship is defined as independent observations
...
functional_form_model = 'GB1959kjf'
...
cat('  Greenberg\n')
cat('  Fixed jam density (GB1959kjf)\n')
cat('\n')
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + I(V2*log(k_jam/V2)), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
