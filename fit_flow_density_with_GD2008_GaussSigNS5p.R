fit_flow_density_with_GD2008_GaussSigNS5p = function(traffic_data, ngrid, upper_density, output_files) {
...
tryCatch(
  ...
  { init_model_obj = gamlss(V3 ~ 0 + V2 + I(V2*log(V2)), sigma.formula = ~ 1, family = NO(), data = traffic_data)
    ...
    model_formula = quote(gamlss(V3 ~ 0 + I(V2*log((p[1] + p[2])/(p[1] + V2))), sigma.formula = ~ ns(V2, df = 4), family = NO(),
    ...
    model_obj = gamlss(V3 ~ 0 + I(V2*log((par1 + par2)/(par1 + V2))), sigma.formula = ~ ns(V2, df = 4), family = NO(), data = traffic_data,
