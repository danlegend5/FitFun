fit_flow_density_with_GZ1961A_SEP3SigNS5pNuNS3pTauL2p = function(traffic_data, ngrid, upper_density, output_files) {
...
#              script "FitFun.R". The model component for the functional form of the flow-density relationship is the Gazis model A (GZ1961A). The
...
tryCatch(
  { model_obj = gamlss(V3 ~ 0 + sqrt(V2) + V2, sigma.formula = ~ ns(V2, df = 4), nu.formula = ~ ns(V2, df = 2), tau.formula = ~ 1 + V2, family = SEP3(),
...
cat('  v_bw*sqrt(k_jam):', 0.5*model_obj$mu.coefficients[1], '\n')
cat('  v_bw:            ', -0.5*model_obj$mu.coefficients[2], '\n')
...
tryCatch(
  { write_fit_summary(output_files[1], 'Flow.Density', ntraffic_data, data_min_density, data_max_density, data_min_flow, data_max_flow,
    ...
    cat('######################################################################################################################\n',
        ...
        '######################################################################################################################\n',
        0.5*model_obj$mu.coefficients[1], '           # v_bw*sqrt(k_jam)\n',
        -0.5*model_obj$mu.coefficients[2], '           # v_bw\n',
