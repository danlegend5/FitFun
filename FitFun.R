...
#              & Ambuhl, 2023, Transportation Research Part C, ???, ???). It can be used to fit EFD data in either flow-density or speed-density form
#              with any model from an extensive list of fundamental diagram models. The input EFD data, which are typically generated by a single
...
#              ???, ???). The models are fit to the EFD data using maximum likelihood (ML) estimation, or maximum penalised likelihood (MPL) estimation,
#              as appropriate (for more details see Bramich, Menendez & Ambuhl, 2023, Transportation Research Part C, ???, ???). The script also
#              computes the values of two different information criteria for any model fit, which enables model comparisons based on the Principle of
#              Parsimony to be performed (see below).
...
#              Transportation Research Part C, ???, ???). However, this script does not provide any analysis of the NQR with regards to fit quality,
...
#                          this argument are (Bramich, Menendez & Ambuhl, 2023, Transportation Research Part C, ???, ???):
#
